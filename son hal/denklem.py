...
class Ui_DenklemEkle(object):
    def setupUi(self, DenklemEkle):
        ...
        self.btn_sil.clicked.connect(self.temizle)
    ...
    """def d_kaydet(self):
        ...
        self.btn_cikis.setEnabled(True)
        #iste.denklem=self.lbl_denklem.text()
        print(liste.denklem)"""
    ...
    def e_ekle(self):
        ...
        if len(self.txt_ksayi.text()) > 0 and len(self.txt_us.text()) > 0:
            ...
            liste.denklem=self.lbl_denklem.text()

            if self.comboBox.currentText()=="-":
                ks ="-"+ self.txt_ksayi.text()
            else:
                ks = self.txt_ksayi.text()
            ...
            """with open("denklem2.txt", "w") as dosya:
                dosya.write(str(liste.dl))"""
    ...
    def temizle(self):
        self.lbl_denklem.setText("")
        """if os.path.isfile("denklem2.txt"):
            os.remove("denklem2.txt")"""
        liste.dl.clear()
        liste.st=0
        liste.denklem=""
    ...
    def st_ekle(self):
        ...
        """with open("denklemSbtSayi.txt", "w") as dosya:
            dosya.write(str(self.txt_sabit_deger.text()))"""

        if self.comboBox2.currentText() == "-":
            sbt_trm = "-" + self.txt_us.text()
        else:
            sbt_trm = self.txt_us.text()
    ...
    def retranslateUi(self, DenklemEkle):
        ...
        self.label_2.setText(
            _translate("DenklemEkle","--> Bu kutucuğu denklemin sabit terimini girmek için kullanınız \n      Sabit Değer Bir Kez Eklenebilir"))
